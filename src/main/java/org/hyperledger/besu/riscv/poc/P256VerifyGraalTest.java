...
/**
 * Test case for P-256 (secp256r1) signature verification using BoringSSL. Exercises signature
 * verification operations in a GraalVM native-image context. Tests valid signatures, malleated
 * signatures, and invalid inputs.
 */
public class P256VerifyGraalTest {

  private P256VerifyGraalTest() {}

  private static final Bytes DATA =
      Bytes.fromHexString(
  ...
  private static final Bytes INVALID_PUBLIC_KEY =
      Bytes.fromHexString(
  ...
  private static void testValidSignature(byte[] dataHash) {
    byte[] input =
        createInput(
            dataHash,
            SIGNATURE_R.toArrayUnsafe(),
            SIGNATURE_S.toArrayUnsafe(),
            PUBLIC_KEY.toArrayUnsafe());
  ...
  /**
   * Test verification of a malleated signature (signature with s' = order - s). Both s and s'
   * should be valid for secp256r1.
   */
  private static void testMalleatedSignature(byte[] dataHash) {
    ...
    if (result.status != 0) {
      throw new RuntimeException(
          "Malleated signature verification failed! Status: " + result.status);
    }
  ...
  private static void testInvalidSignature(byte[] dataHash) {
    byte[] input =
        createInput(
            dataHash,
            INVALID_SIGNATURE_R.toArrayUnsafe(),
            SIGNATURE_S.toArrayUnsafe(),
            PUBLIC_KEY.toArrayUnsafe());
  ...
  private static void testInvalidPublicKey(byte[] dataHash) {
    byte[] input =
        createInput(
            ...
            SIGNATURE_R.toArrayUnsafe(),
            SIGNATURE_S.toArrayUnsafe(),
            INVALID_PUBLIC_KEY.toArrayUnsafe());
  ...
  /**
   * Helper method to create 160-byte input array. Format: hash(32) + r(32) + s(32) + pubkey(64) =
   * 160 bytes
   *
  ...
  private static byte[] createInput(byte[] hash, byte[] r, byte[] s, byte[] pubkey) {
    if (hash.length != 32 || r.length != 32 || s.length != 32 || pubkey.length != 64) {
      throw new IllegalArgumentException(
          String.format(
              "Invalid component lengths: hash=%d, r=%d, s=%d, pubkey=%d",
